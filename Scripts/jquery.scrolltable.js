...
$j.widget( "ui.scrolltable", {
	...
	destroy: function(){
		...
		$self.find('>thead th, >thead td')
			.each(function(){
				$j(this).prop('colspan', $j(this).data('colspan'));
			})
			.add($self.find('>tbody>tr').eq(0).find('td')).width(function(){
				return $j(this).data('prevWidth') || 'auto';
			});
		...
		$j.Widget.prototype.destroy.call(self);
	},
	...
	_convertTable: function($container){
		...
		var $head = $j('<table class="st-head-table" cellpadding="0" cellspacing="0" border="0" />').append($container.find('>thead')).css('width', '100%');
		var $body = $j('<table class="st-body-table" cellpadding="0" cellspacing="0" border="0" />').append($container.find('>tbody')).css('width', '100%');
		...
		$container.find('.st-head thead th, .st-head thead td').each(function(){
			$j(this).data('colspan', $j(this).prop('colspan')).removeProp('colspan');
		});
	},
